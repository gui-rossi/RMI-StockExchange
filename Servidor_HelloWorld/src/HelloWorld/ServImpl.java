...
public class ServImpl extends UnicastRemoteObject implements InterfaceServ {
	...
	synchronized public void encerrarNegocio(int id, String empresa, String tipo_negocio) throws RemoteException, InterruptedException {
		...
		for (compra_venda buyers_sellers : negocios) {
			...
			/*se o negocio ja foi vendido ou comprado*/
			else {
	 ...
	 * tempo: tempo maximo até o pedido expirar
			...
			if (clientes.id == id) {
				...
				for (cotacoes_detem detem : clientes.cotacoes_detem) {
					...
					if(detem.acao.contains(empresa)) {
						...
						if (detem.qtde < qtde) {
							System.out.println(clientes.nome + " quer vender " + qtde + " acoes e só tem " + detem.qtde + " acao cancelada");
						}
			...
			if (clientes.id == id) {
				...
				for (cotacoes_detem detem : clientes.cotacoes_detem) {
					...
					if(detem.acao.contains(empresa)) {
						...
						//verifico se o acionista ja tem pedidos de compra, pego o pior caso (valor minimoMaximo, que no caso de compras é Maximo e multiplico pela quantidade de ativos)
	...
	@Override
	public void checaNegocios () throws RemoteException{
						...
						cotacao_atual <= compras_vendas_aux.valor_MinimoMaximo) 
					{
						...
						if (compras_vendas.qtde_acoes < compras_vendas_aux.qtde_acoes) {
							for (cliente clientes : acionistas) {
								...
								if (clientes.id == compras_vendas.id_cliente) {
									clientes.dinheiro = clientes.dinheiro + (cotacao_atual * compras_vendas.qtde_acoes);
									for(cotacoes_detem detem : clientes.cotacoes_detem) {
										if (compras_vendas.empresa.contains(detem.acao)) {
											//tira a quantidade de acoes do nome dele
											detem.qtde = detem.qtde - compras_vendas.qtde_acoes;
											//se terminou com 0 acoes, remove da lista
											if (detem.qtde <= 0)
												clientes.cotacoes_detem.remove(detem);
										}
									}
									//notifico o vendedor que ele vendeu acoes
									clientes.refCli.notificar(clientes.nome + " vendeu " + compras_vendas.qtde_acoes + " da " + compras_vendas.empresa);
								}
								//achou comprador: tira dinheiro da conta 
								else if (clientes.id == compras_vendas_aux.id_cliente) {
									//booleano de controle para criar nova instancia de classe ou nao
									boolean ja_detia_acao = false;
									clientes.dinheiro = clientes.dinheiro - (cotacao_atual * compras_vendas.qtde_acoes);
									for(cotacoes_detem detem : clientes.cotacoes_detem) {
										if (compras_vendas_aux.empresa.contains(detem.acao)) {
									...
									if (!ja_detia_acao) {
										cotacoes_detem det = new cotacoes_detem(compras_vendas_aux.empresa, compras_vendas.qtde_acoes);
										clientes.cotacoes_detem.add(det);
									}

									clientes.refCli.notificar(clientes.nome + " comprou " + compras_vendas.qtde_acoes + " da " + compras_vendas_aux.empresa + ", ainda quero comprar " + (compras_vendas_aux.qtde_acoes - compras_vendas.qtde_acoes));
								}
							}
							//atualiza o pedido de compra do comprador, agora ele precisa comprar menos acoes 
							compras_vendas_aux.qtde_acoes = compras_vendas_aux.qtde_acoes - compras_vendas.qtde_acoes;
						...
						//2 se o vendedor vende mais acoes do que o comprador compra
							...
							for (cliente clientes : acionistas) {
								...
								if (clientes.id == compras_vendas.id_cliente) {
									clientes.dinheiro = clientes.dinheiro + (cotacao_atual * compras_vendas_aux.qtde_acoes);
									for(cotacoes_detem detem : clientes.cotacoes_detem) {
										if (compras_vendas.empresa.contains(detem.acao)) {
											detem.qtde = detem.qtde - compras_vendas_aux.qtde_acoes;
										}
									}
									//notifico o vendedor que ele vendeu acoes
									clientes.refCli.notificar(clientes.nome + " vendeu " + compras_vendas_aux.qtde_acoes + " da " + compras_vendas.empresa + ", ainda quero vender " + (compras_vendas.qtde_acoes - compras_vendas_aux.qtde_acoes));
								}
								//achou comprador: tira dinheiro da conta, coloca mais acoes no nome dele
								else if (clientes.id == compras_vendas_aux.id_cliente) {
									boolean ja_detia_acao = false;
									/*CHECAR SE ELE TEM DINHEIRO SUFICIENTE PRA REALIZAR A COMPRA*/
									clientes.dinheiro = clientes.dinheiro - (cotacao_atual * compras_vendas_aux.qtde_acoes);
									for(cotacoes_detem detem : clientes.cotacoes_detem) {
										if (compras_vendas_aux.empresa.contains(detem.acao)) {
											detem.qtde = detem.qtde + compras_vendas_aux.qtde_acoes;
									...
									//se nao tinha a acao na lista, adiciona na lista
									if (!ja_detia_acao) {
										cotacoes_detem det = new cotacoes_detem(compras_vendas_aux.empresa, compras_vendas_aux.qtde_acoes);
										clientes.cotacoes_detem.add(det);
									}

									clientes.refCli.notificar(clientes.nome + " comprou " + compras_vendas_aux.qtde_acoes + " da " + compras_vendas_aux.empresa);
								}
							}
							//atualiza o pedido de venda do vendedor, agora ele precisa vender menos acoes 
							compras_vendas.qtde_acoes = compras_vendas.qtde_acoes - compras_vendas_aux.qtde_acoes;
						...
						//3 se o vendedor e o comprador compram acoes de mesma quantidade
							...
							for (cliente clientes : acionistas) {
								...
								//achou comprador: tira dinheiro da conta, coloca mais acoes no nome dele
									...
									clientes.refCli.notificar(clientes.nome + " comprou " + compras_vendas.qtde_acoes + " da " + compras_vendas_aux.empresa);
								}
							}
			...
			for (cotacoes_notificacao notificacao : clientes.cotacoes_notificacao) {
				...
				for (int i = 0; i < cotacao_global.num_acoes; i++) {
					...
					if (cotacao_global.acoes[i].contains(notificacao.cotacoes)) {
						...
						/*se ele possui uma notificacao de ganho e o proximo valor de cotacao for maior que o limite de ganho*/
						else if (notificacao.tipo.contains("ganho") && cotacao_global.precos[i] + cotacao_global.multiplicador_importancia[i] > notificacao.limite) {
							clientes.refCli.notificar(clientes.nome + " as acoes da " + notificacao.cotacoes + " atingiram o limite de " + notificacao.tipo + " (Cotacao atual: " + cotacao_global.precos[i] + ")");
							clientes.cotacoes_notificacao.remove(notificacao);
						}
					}
    ...
    public void registrarInteresse(String nome, int id, InterfaceCli referenciaCliente) throws RemoteException{
    	...
    	System.out.println(nome + ", interesse no mercado de ações registrado");
    }
     ...
     * o que significa que empresas grandes tem valores que oscilam mais rapidos, ja que o valor incrementado é maior.
